...
Streamlit-дашборд, который по тикеру (или загруженному 10-K) собирает понятную **карточку компании на языке новичка**. Источники — SEC EDGAR (XBRL + текст отчётов) и finviz; нарратив генерируется через **Anthropic Claude**.
...
```bash
git clone https://github.com/YOUR_USERNAME/business-analyzer.git
